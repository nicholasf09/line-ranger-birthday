function main(){
    ...
    for(var i = 0; i <= 360; i++){
        ...
        circle_vertex.push(x,y,255,0,255);
    }
    ...
    for(var i = 0; i <= 360; i++){
        ...
        var x = Math.cos(theta);
        var y = Math.sin(theta);
        circle1_vertex.push(x-0.675,y,255,255,255);
    }

    var CIRCLE1_VERTEX = GL.createBuffer();
    GL.bindBuffer(GL.ARRAY_BUFFER, CIRCLE1_VERTEX);
    GL.bufferData(GL.ARRAY_BUFFER, new Float32Array(circle1_vertex),GL.STATIC_DRAW);

    //FACES lingkaran besar
    var circle_faces = [];
    for(var i = 1; i <= 360; i++){
    ...
    //FACES lingkaran kecil
    var circle1_faces = [];
    for(var i = 1; i <= 360; i++){
        circle1_faces.push(0,i,i+1);
    }
    circle1_faces.push(0,361,1);
    
    var CIRCLE1_FACES = GL.createBuffer();
    GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, CIRCLE1_FACES);
    GL.bufferData(GL.ELEMENT_ARRAY_BUFFER, new Uint16Array(circle1_faces),GL.STATIC_DRAW);
